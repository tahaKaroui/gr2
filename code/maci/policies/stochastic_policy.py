...
class StochasticNNConditionalPolicy(NNPolicy, Serializable):
    """Stochastic neural network policy.
    StochasticNNConditionalPolicy is a class representing a policy for an agent in a multi-agent reinforcement learning
    setting.
    The policy is implemented as a neural network that takes in the agent's observation and its own action as input,
    and outputs the action to be taken by the agent's opponent.
    The policy is "stochastic" in the sense that it samples the opponent's action from a distribution parameterized by
    the neural network.
    The policy is "conditional" in the sense that it depends on both the agent's observation and its own action.
    The u_range parameter determines the range of the distribution from which the opponent's action is sampled,
    while the squash parameter determines whether the output of the neural network should be squashed (i.e., transformed
    to lie within a certain range) before sampling.
    The hidden_layer_sizes parameter specifies the sizes of the hidden layers in the neural network.
    The sampling parameter determines whether the policy should sample the opponent's action or simply output the mean
    of the distribution.
    """
...
class TahaStochasticNNConditionalPolicy(NNPolicy, Serializable):
    """Stochastic neural network policy.
    StochasticNNConditionalPolicy is a class representing a policy for an agent in a multi-agent reinforcement learning
    setting.
    The policy is implemented as a neural network that takes in the agent's observation and its own action as input,
    ...
    def __init__(self,
        ...
        super(TahaStochasticNNConditionalPolicy, self).__init__(
            env_spec, self._observation_ph, self._opponent_actions, self._name)
